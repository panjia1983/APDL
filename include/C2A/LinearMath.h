...
public:
	inline Coord3D() {  }
	inline Coord3D(const Coord3D& t)
	{
		*this = t;
	}
	...
	inline void Set_X(Real x)
	{
		val[0] = x;
	}
	inline void Set_Y(Real y)
	{
		val[1] = y;
	}
	inline void Set_Z(Real z)
	{
		val[2] = z;
	}
...
public:
	...
	inline Quaternion(Real t[])
	{
		Set_Value(t);
	}
	// Creates a quaternion [x,y,z,0]
	inline Quaternion(const Coord3D& t)
	{
		Set_Value(t);
	}
	...
	inline Real Length() const
	{
		return Real(sqrt(val[0] * val[0] + val[1] * val[1] + val[2] * val[2] + val[3] * val[3]));
	}
	...
	inline Real Angle()
	{
		if(val[3] < -1.0 || val[3] > 1.0) return 0.0;
...
public:
	...
	inline Matrix3x3(Real v[3][3])
	{
		Set_Value(v);
	}
	inline Matrix3x3(Real v[])
	{
		Set_Value(v);
	}
...
public:
	...
	// Get/set routines
	inline const Coord3D& Translation() const
	{
		return T;
	}
	inline Coord3D& Translation()
	{
		return T;
	}
	inline const Matrix3x3& Rotation() const
	{
		return R;
	}
	inline Matrix3x3& Rotation()
	{
		return R;
	}
	inline const Quaternion Quaternion_()
	{
		return R.Quaternion_();
	}
	
	
	inline void Set_Value(const Transform& m)
	{
	...
	inline void Set_Value(const Matrix3x3& r, const Coord3D& t)
	{
		R = r;
		T = t;
	}
	inline void Set_Rotation(const Matrix3x3& r)
	{
		R = r;
	}
	...
	inline void Set_Translation(const Coord3D& t)
	{
		T = t;
	}
