...
#include <snopt/snopt.hh>
#include "../snoptProblem.h"


double* user_data;
const struct svm_model* user_model;
				...
				doublereal ru[],    integer *lenru )
{
	...
	double exp_value = 0;
	for(int i = 0; i < *n; ++i) exp_values[i] = 0;
	for(int i = 0; i < l; ++i)
	{
		double v = user_model->sv_coef[0][i] * Kernel::k_function(user_x_node, user_model->SV[i], user_model->param);
		exp_value += v;
		for(int j = 0; j < *n; ++j)
			exp_values[j] += v * user_model->SV[i][j].value;
	}
	...
	if(*needF > 0)
	{
		F[0] = 0;
		for(int i = 0; i < *n; ++i)
			F[0] += (x[i] - user_data[i]) * (x[i] - user_data[i]);

		F[1] = exp_value - user_model->rho[0];
		F[1] *= user_model->label[0];
	}
	...
	if(*needG > 0)
	{
		*neG = 0;
		for(int i = 0; i < *n; ++i)
		{
			G[*neG] = 2 * (x[i] - user_data[i]);
			*neG = *neG + 1;
		}

		for(int i = 0; i < *n; ++i)
		{
			G[*neG] = - 2 * user_model->param.gamma * (x[i] * exp_value - exp_values[i]);
			G[*neG] *= user_model->label[0];
			*neG = *neG + 1;
		}
	}
...
int distanceF(integer    *Status, integer *n,    doublereal x[],
			  integer    *needF,  integer *neF,  doublereal F[],
			  integer    *needG,  integer *neG,  doublereal G[],
			  char       *cu,     integer *lencu,
			  integer    iu[],    integer *leniu,
			  doublereal ru[],    integer *lenru )
{
	...
	for(int i = 0; i < l; ++i)
		F[1] += user_model->sv_coef[0][i] * Kernel::k_function(user_x_node, user_model->SV[i], user_model->param);

	F[1] -= user_model->rho[0];
	F[1] *= user_model->label[0];
...
double dist_to_decision_boundary(const struct svm_model* model, const struct svm_node* x_node, double* upper, double* lower, struct svm_node* x_res)
{
	...
	double dist = 0;
	for(int i = 0; i < n; ++i)
		dist += (x_node[i].value - x[i]) * (x_node[i].value - x[i]);
...
double dist_to_decision_boundary_with_gradient(const struct svm_model* model, const struct svm_node* x_node, double* upper, double* lower, struct svm_node* x_res)
{
	...
	delete [] user_x_node; user_x_node = NULL;
	delete [] user_data; user_data = NULL;
	delete [] exp_values; exp_values = NULL;

	return dist;
}

const struct svm_node* user_fixed_node; // set to x_node
double user_constant; 

int distanceF_free(integer    *Status, integer *n,    doublereal x[],
				   integer    *needF,  integer *neF,  doublereal F[],
			       integer    *needG,  integer *neG,  doublereal G[],
			       char       *cu,     integer *lencu,
			       integer    iu[],    integer *leniu,
			       doublereal ru[],    integer *lenru )
{
	...
	F[0] = -Kernel::k_function(user_fixed_node, user_x_node, user_model->param) + user_constant * svm_predict_values_twoclass(user_model, user_x_node);
...
double dist_to_decision_boundary_constrain_free(const struct svm_model* model, double w_norm, const struct svm_node* x_node, double* upper, double* lower, struct svm_node* x_res)
{
	...
	prob.setFNames(Fnames, nFnames);
	prob.setProbName("distance0");
	prob.setUserFun(distanceF_free);
	...
	user_constant = svm_predict_values_twoclass(model, x_node) / w_norm;
			       ...
			       doublereal ru[],    integer *lenru )
{
	...
	F[0] = -Kernel::k_function(user_fixed_node1, user_x_node, user_model->param) - Kernel::k_function(user_fixed_node2, user_x_node, user_model->param);

	return 0;
}


void feature_space_midpoint(const struct svm_model* model, const struct svm_node* x_node, const struct svm_node* y_node, struct svm_node* midpoint, double* upper, double* lower)
{
	snoptProblem prob(0);
	...
	prob.setUserFun(midpointF_free);
